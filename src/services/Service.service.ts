...
export const ServiceService = (
  ctx?: NextPageContext
): ServiceServiceResponseI => {
  const getServiceWeight = async (
    cable_type: string
  ): Promise<AxiosResponse> => {
    const res = await useHttp(ctx).post(endpoints.getServiceWeight, {
      cable_type,
    });
    return res;
  };

  const getServiceDecoding = async (
    cable_type: string
  ): Promise<AxiosResponse> => {
    const res = await useHttp(ctx).post(endpoints.getServiceDecoding, {
      cable_type,
    });
    return res;
  };
