...
export const SertificateService = (ctx?: NextPageContext): SertificateServiceResponseI => {
  const getSertificate = async (): Promise<sertificateI[]> => {
    const res = await useHttp(ctx).get<sertificateAnswsI>(endpoints.getSertificate);
